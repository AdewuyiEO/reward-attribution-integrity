...
```
Detection quality      ROC-AUC 0.735  ·  PR-AUC 0.078  ·  6.0% precision in top 1% of scores
Operating point        blocks 573 entities, catching 1% of estimated fraudulent traffic while affecting 0.72% of legitimate entities
...
### 1b. An evidence gate that polices the system's own confidence

![evidence gate](docs/figures/evidence_gate.png)

A 115-click IP can look extremely anomalous — but the detectability floor (below) says we cannot *prove* a single entity fraudulent from its own record below ~1,211 clicks. So each entity carries an **evidence tier**, and the actionable rank is `fraud_priority = fraud_score × volume_confidence`, not the raw anomaly score:

| Tier | Meaning | Priority |
|---|---|---|
| **proven** | above the detectability floor | full |
| **ring (collective)** | member of a coordinated cluster — evidence is the coordination, not the volume | full |
| **suspected** / **unproven** | anomalous but below the floor | demoted to a watchlist |

The nuance that makes this defensible: an *isolated* low-volume entity is demoted, but a *ring* of identical low-volume bots keeps full priority — because the coordination across many entities is evidence that doesn't depend on any single one's volume. This turns the detectability floor from a README caveat into an enforced property of the output.
...
> Recommended operating threshold **0.98**. Blocks **573** entities, catching **1%** of estimated fraudulent traffic at **7%** precision, affecting **0.72%** of legitimate entities. Estimated net value protected: **$792** over the analysed period.

Precision of 7% is acceptable *by design*: the fraud caught is high-volume and expensive, the false positives are low-volume and cheap. Optimising precision would have meant catching less of what actually costs money.
...
```
n = ln(0.05) / ln(1 − 0.00247) ≈ 1,211 clicks
```

At a 0.247% baseline conversion rate, **an entity needs ~1,211 clicks before zero conversions is statistically surprising.**
...
It also explains the honest gap in the results — PR-AUC is **0.078** against the statistical proxy, because the proxy can only recognise entities above the floor. **The detector finds fraud the proxy cannot prove.** Reporting only the flattering number would hide that.

### Two evaluation regimes — read this before comparing numbers

The headline metrics above (ROC-AUC 0.998, 58 entities, $471) come from the **synthetic-validation regime**: a generated dataset with *planted* fraud archetypes, where ground truth is known exactly. They validate that the detector mechanism works. Anyone can reproduce them with `make synth && make run`.

On the **real TalkingData regime**, the operating-point metrics are lower and messier — the proxy only sees above-floor fraud, real traffic is noisier, and there are no planted labels. That is expected, not a regression. When you run on `train.csv`, record your real numbers here:

| Metric | Synthetic (planted truth) | Real (statistical proxy) |
|---|---|---|
| ROC-AUC | 0.998 | 0.735 |
| Entities blocked @ rec. threshold | 58 | 573 |
| Precision / recall @ rec. threshold | 34% / 95% | 7% / 1% |
| Legit entities affected | 0.20% | 0.72% |

Presenting both regimes side by side is stronger than either alone: synthetic proves the mechanism, real proves it scales.
...
notebooks/   01_eda.ipynb — exploration with CIs, leakage demo, detectability floor
docs/        analysis_writeup · architecture · ab_test_design · CODE_DEFENSE · VIDEO_SCRIPT
scripts/     synthetic data generator (planted fraud) · figure generation
tests/       22 tests, including leakage guards and statistical correctness
```
...
22 tests covering statistical correctness (Wilson intervals, PSI, KS, BH-FDR), detector behaviour, cost-model guardrails, and **three leakage guards** that fail the build if a label column ever reaches the feature set.
